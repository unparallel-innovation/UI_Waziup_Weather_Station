							...
							spi_t spi, usart1_t usart1,	twi_t twi, usb_t usb)
{
	...
	if (period != SLEEP_FOREVER)
	{
		wdt_enable(period);					// Desabilitar aqui e forçar no setup
		WDTCSR |= (1 << WDIE);	
	}
	
	lowPowerBodOn(SLEEP_MODE_PWR_DOWN);		//Change by Sérgio Rocha
...
ISR (WDT_vect)
{
	...
	wdt_disable();				// DESABILITAR E EXPERIMENTENTAR por prints para ver se garantir que dorme 8 segunos 
}
