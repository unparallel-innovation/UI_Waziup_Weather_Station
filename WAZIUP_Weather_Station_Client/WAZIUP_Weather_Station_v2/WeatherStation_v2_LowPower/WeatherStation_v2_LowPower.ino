...
void setup() 
{
  ...
  digital.pinMode(21, INPUT_PULLUP, RISING);    // Select Rain interrupt
  digital.pinMode(22, INPUT_PULLUP, RISING);    // Select Wind interrupt
    
  timer.setTimer(sleepTime);  // Set Low Power Timer wake up in milliseconds
...
float get_wind_speed(int wind)
{
  ...
  if (deltaTime > 0)
  {
      windSpeed =  ((float)wind / 2) / deltaTime;   // divide because the interrupt is detecting the rising and falling edge
...
float get_rain(int rain)
{
  ...
  rainClicks= rain/2;     // divide because the interrupt is detecting the rising and falling edge
...
void loop() 
{
  ...
  //-------------------------- Message to be sent -------------------------------
    ...
    r_size += sprintf((char*)message + app_key_offset + r_size, "/WS/%s", dtostrf(getWindSpeed, 3, 2, auxBuf));
  ...
  unsigned long new_sleep=0;
      ...
      do
      {
          // Time before sleep
          now = rtc.now();
          before_time = now.unixtime();
          
          #ifdef SNOOZE_HIBERNATE
            snooze_who = Snooze.hibernate(config_teensy);
          #endif
          
          #ifdef SNOOZE_SLEEP
            snooze_who = Snooze.sleep(config_teensy);
          #endif
          
          #ifdef SNOOZE_DEEPSLEEP
            snooze_who = Snooze.deepSleep(config_teensy);
          #endif
          ...
          after_time = now.unixtime();
                ...
                Serial.println(after_time);
          #endif
  
          current_time += ((after_time-before_time)*1000);
  
          new_sleep=(sleepTime-current_time);
  
          #ifdef PRINT
                Serial.println(F("Current Sleep: "));
                Serial.println(current_time);
                Serial.println(F("New Sleep: "));
                Serial.println(new_sleep);
          #endif
  
          if(new_sleep>0)
            timer.setTimer(new_sleep);  // Set Low Power Timer wake up in milliseconds
          else
