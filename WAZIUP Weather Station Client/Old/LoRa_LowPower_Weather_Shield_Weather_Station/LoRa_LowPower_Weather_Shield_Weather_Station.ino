...
#include "WeatherConfig.h"      //  Includes the "SparkFunMPL3115A2.h" & "SparkFunHTU21D.h" libraries

WeatherConfig myConfigWeather;  //  for Pressure sensor & Humidity sensor respectively
...
unsigned int idlePeriodInMin = 0.3; // The device will wake up every 0.3 minutes
...
// Change here the AppKey, But if GW checks for AppKey, must be in the AppKey
...
float get_gust()
{
  ...
  gustTime /= 1000.0; //Covert to seconds

  if (gustTime > 300) //300s==5 minutos
  {
...
// Returns the rain 5 mins (Function similar to rainIRQ();)
float get_rain()
{
  float rTime = millis() - lastRainCheck;

  rTime /= 1000.0;  //Convert to seconds
